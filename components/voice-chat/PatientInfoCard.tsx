...
const InfoRow = ({
	...
	value ? (
		<div className="flex items-center gap-2">
			<Label className="min-w-[120px] text-muted-foreground">
				{label}:
			</Label>
				...
				{hasMoreVitals && (
					...
					>
						{showAll
							? "Show Less"
							: `Show ${
									vitalSigns.length - INITIAL_DISPLAY_COUNT
							  } More`}
					</Button>
				)}
			</Section>
		...
		<Card className="h-full border-0">
			...
			<CardContent className="space-y-6 overflow-y-auto max-h-[calc(100vh-200px)]">
				...
				<Section title="Admission Info">
					...
					<InfoRow
						label="Admitted"
						value={moment(selectedPatient.admissionDate).format(
							"MMM DD, YYYY"
						)}
					/>
					<InfoRow
					...
					{selectedPatient.O2Therapy && (
						<InfoRow
							label="O2 Therapy"
							value={selectedPatient.O2Therapy}
						/>
					)}
				</Section>
				...
				{selectedPatient.contactInfo &&
					selectedPatient.contactInfo.length > 0 && (
						<Section title="Emergency Contacts">
							{selectedPatient.contactInfo.map(
								(contact, index) => (
									<div
										key={index}
										className="bg-muted p-3 rounded-md mb-2"
					...
					(selectedPatient?.inputs?.length > 0 ||
						...
						<Section title="I/O Balance">
							{selectedPatient.netFluidBalance !== undefined && (
								...
								>
									Net Balance:{" "}
									{selectedPatient.netFluidBalance} mL
								</Badge>
							)}
							...
							{selectedPatient.outputs?.map((output, index) => (
								...
								>
									...
									<span className="text-xs text-muted-foreground">
										{moment(output.timestamp).format(
											"HH:mm"
										)}
									</span>
								</div>
